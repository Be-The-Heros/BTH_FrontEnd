...
const Helpers = {
  ...
  getTickers: (markets) => {
    ...
    //     avg_price: '0.0',
    //     price_change_percent: `${signPrefix}${Math.abs(
  ...
  getBalances: () => {
    const getBalanceValue = (value, precision) =>
      (((Math.random() > 0.5 ? 1 : -1) * Math.random()) / 10 + +value).toFixed(
        precision
      );
    const getLockedValue = (value, precision) =>
      (Math.random() / 10 + +value).toFixed(precision);
  ...
  getDepthIncrement: (sequence) => {
    const delta =
      getRandomArbitrary(1, 100, 2) *
      (1 + Math.cos((2 * Math.PI * Date.now()) / 1000 / 3600));
