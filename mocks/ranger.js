...
const timeToPrice = (time) => {
  return (
    minDay +
    ((maxDay - minDay) / 2) * (1 + Math.cos((time / fakePeriod) * 2 * Math.PI))
  );
};

const timeToVolume = (time, periodInSeconds) => {
  return (
    ((maxDay * 10) / 2) * (1 + Math.cos((time / fakePeriod) * 2 * Math.PI))
  );
};
...
class RangerMock {
  ...
  subscribe(ws, streams) {
    ...
    ws.send(
      JSON.stringify({
        success: { message: "subscribed", streams: ws.streams },
      })
    );
  }
  unsubscribe(ws, streams) {
    ...
    ws.send(
      JSON.stringify({
        success: { message: "unsubscribed", streams: ws.streams },
      })
    );
  }
}
